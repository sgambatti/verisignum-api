...
const [copyStatus, setCopyStatus] = useState({ error: null });
...
setActiveTab(tab);
setIsMobileMenuOpen(false); // Fecha o menu automaticamente no telemóvel ao clicar nalguma coisa
}
...
const handleShieldSubmit = async (e: React.FormEvent): Promise => {
e.preventDefault();
...
const handleLensScan = async (e: React.FormEvent): Promise => {
e.preventDefault();
if (!lensFile) return;
...
const sendMessageToGemini = async (): Promise => {
...
Olá, {clientData.name}. Confirme a seleção do seu plano para aceder à API.
