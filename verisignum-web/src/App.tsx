...
export default function App() {
  ...
  // A sua chave de acesso gerada no Render
  ...
  const handleShieldSubmit = async (e: React.FormEvent) => {
    ...
    setShieldStep('A ligar à API segura no Render...');
    setCopyStatus({ hash: false, key: false, error: null });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || `Erro HTTP ${response.status}: Acesso Negado ou Falha no Servidor.`);
      }
  ...
  const handleLensScan = async (e: React.FormEvent) => {
    ...
    setIsScanning(true);
    setScanResult(null);
    setScanStep('A comunicar com o servidor forense na nuvem...');
    setCopyStatus({ hash: false, key: false, error: null });
    ...
    try {
      ...
      setScanStep('A extrair e validar metadados criptográficos C2PA...');
      ...
      setScanStep('A analisar estrutura de píxeis...');
      ...
      if (verifyData.has_c2pa) {
        ...
        let anomalies = ['Nenhuma assinatura C2PA encontrada.'];
  ...
  const sendMessageToGemini = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'assistant', text: 'Erro de ligação ao servidor.' }]);
    } finally {
          ...
          <form onSubmit={handleShieldSubmit} className="space-y-4 max-w-xl">
            <div className="bg-[#161b22] border border-[#30363d] p-6 rounded-xl space-y-6 shadow-sm">
                <div>
                  ...
                  <p className="text-sm text-gray-400">Insira a sua Chave de API e faça o upload de um arquivo.</p>
                </div>

                <div className="bg-[#0d1117] border border-emerald-500/30 p-4 rounded-lg flex flex-col gap-2 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-emerald-500"></div>
                  <label className="text-xs font-semibold text-emerald-400 flex items-center gap-1"><Key size={14}/> Credencial de Acesso (API Key)</label>
                  <input 
                    type="text" 
                    value={apiKey} 
                    onChange={(e) => setApiKey(e.target.value)} 
                    placeholder="Cole aqui a chave gerada no /docs do Render..."
                    className="w-full bg-[#161b22] border border-[#30363d] rounded-lg p-2.5 text-sm text-emerald-400 font-mono focus:ring-1 focus:ring-emerald-500 outline-none" 
                    required 
                  />
                </div>
                ...
                <div className="border-2 border-dashed border-[#30363d] p-8 text-center cursor-pointer bg-[#0d1117] rounded-xl hover:border-indigo-500 transition-all">
                  ...
                  <label htmlFor="shield-file-input" className="text-white text-sm cursor-pointer px-4 py-2 bg-[#21262d] border border-[#30363d] rounded-lg hover:bg-[#30363d]">
                    {shieldFile ? 'Trocar Mídia' : 'Selecionar Mídia'}
                  </label>
                </div>
