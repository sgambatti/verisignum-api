...
app = FastAPI(title="Verisignum API Master", version="3.3.1")
...
async def get_current_client(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    # Validação para Sistemas Externos (Make.com, Zapier, Servidores) usando a API Key fixa
    ...
    # Validação via JWT (Dashboard React / Swagger)
    try:
...
def register_client(name: str, email: str, password: str, db: Session = Depends(get_db)):
    ...
    # Deteta se é o Admin a registar-se e dá ativação automática
...
def register_admin_direct(name: str, email: str, password: str, db: Session = Depends(get_db)):
    """
    Cria uma conta com is_active=True incondicionalmente, pulando a tela de Trial.
    """
    if db.query(Client).filter(Client.email == email).first():
        raise HTTPException(status_code=400, detail="Este e-mail já está registado.")
...
def reset_database(db: Session = Depends(get_db)):
    ...
    Apaga todos os registos para limpar o ambiente, MAS PRESERVA A CONTA DE ADMIN.
    """
    try:
        # Apaga todos os clientes ONDE o email não seja o do administrador principal
        ...
        return {
            "status": "sucesso", 
            "message": f"Limpeza concluída! Todos os clientes de teste foram apagados. A conta Admin ({ADMIN_EMAIL}) foi preservada e está intacta."
        }
    except Exception as e:
        db.rollback()
        raise HTTPException(
