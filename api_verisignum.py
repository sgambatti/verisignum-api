...
Este é o ficheiro principal que o Render carrega. Ele gere as chamadas
...
# Carrega variáveis de ambiente (essencial para chaves de produção no Render)
load_dotenv()
...
def garantir_chaves_criptograficas(cert_path: str, key_path: str):
    ...
    Isto evita que o Render falhe se a pasta /etc/secrets/ estiver vazia.
    """
    if os.path.exists(cert_path) and os.path.exists(key_path):
        return

    print("[VERISIGNUM INFRA]: Chaves não encontradas. A gerar par de chaves temporárias...")
...
async def assinar_midia(
    ...
    client_type: str = Form("web_dashboard_v3") # Parâmetro de controlo de versão
):
    """
    ...
    try:
        ...
        # 2. Configura e garante a existência de chaves criptográficas
        # O Render lerá isto do render.yaml (/etc/secrets/...)
        ...
        # 4. CORREÇÃO CRÍTICA: Instanciar a classe c2pa.Signer
        # Isto resolve o erro "'dict' object has no attribute 'alg'"
        try:
            # Em versões mais recentes do c2pa-python
            signer = c2pa.Signer.from_pem(cert_path, key_path, "es256")
        except AttributeError:
            # Fallback seguro caso 'Signer' não exista no namespace raiz
            from c2pa import Signer
            signer = Signer.from_pem(cert_path, key_path, "es256")
        
        # 5. Executa a injeção criptográfica
        # CORREÇÃO CRÍTICA 2: O parâmetro data_dir foi incluído para resolver o erro "missing 1 required positional argument"
        data_dir = UPLOAD_DIR
        ...
        # 6. Retorna o ficheiro assinado de volta para o Dashboard do utilizador
        return FileResponse(
            path=caminho_saida,
