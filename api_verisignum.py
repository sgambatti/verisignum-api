...
def register_admin_direct(name: str, email: str, password: str, db: Session = Depends(get_db)):
    """
    Cria ou ATUALIZA uma conta incondicionalmente, pulando a tela de Trial e ativando o acesso VIP.
    """
    client = db.query(Client).filter(Client.email == email).first()
    hashed_password = pwd_context.hash(password)
    trial_time = datetime.utcnow() + timedelta(days=3650) # 10 anos de VIP
    
    # Se a conta existir (mesmo bloqueada/inativa), desativamos o bloqueio e forçamos o acesso VIP
    if client:
...
def reset_database(db: Session = Depends(get_db)):
    """
    Limpa a base de dados mantendo o Administrador intocável.
    """
    try:
        # Apaga TODOS os clientes ONDE o email seja DIFERENTE do email do Admin
        registros_apagados = db.query(Client).filter(Client.email != ADMIN_EMAIL).delete()
        db.commit()
        return {
            "status": "sucesso", 
            "message": f"Base de dados limpa. {registros_apagados} registos removidos. A conta Admin foi preservada!"
        }
...
def reset_password(req: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    db.commit()
    # Logica de envio mockada por seguranca
